...
def _get_plugin_list():
    """Return the list of plugins to update."""
    return [
...
def _get_compatible_plugin_revision(repo, repo_latest_commit):
...
def _create_plugin_nix_file(name, owner, repo_name, fetcher, rev, repo):
    """Create a new plugin nix file if it doesn't exist."""
...
def _update_plugin_nix_file(filename, prev_commit_sha, rev, prev_hash, new_hash):
    """Update the plugin nix file with new revision and hash."""
...
def _update_plugin_ruby_dependencies(filename, repo, rev):
    """Update Ruby dependencies for a plugin if it has any."""
...
def _update_single_plugin(plugin):
    """Update a single plugin to its latest compatible revision."""
    ...
    rev = _get_compatible_plugin_revision(repo, repo_latest_commit)

    filename = _nix_eval(f'builtins.unsafeGetAttrPos "src" discourse.plugins.{name}')
    if filename is None:
        filename = _create_plugin_nix_file(name, owner, repo_name, fetcher, rev, repo)
    else:
    ...
    _update_plugin_nix_file(filename, prev_commit_sha, rev, prev_hash, new_hash)
    _update_plugin_ruby_dependencies(filename, repo, rev)
